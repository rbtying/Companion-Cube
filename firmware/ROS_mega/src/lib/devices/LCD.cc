...
LCD::LCD(uint8_t rs, uint8_t rw,
		uint8_t enable, //function overloading lets the list of arguments specify if 4 or 8 data lines are used and if rw is used or tied to gnd.
		uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3, uint8_t d4, uint8_t d5,
		uint8_t d6, uint8_t d7) // I can't eliminate rw and add en2 and be unique.
{
...
void LCD::scrollDisplayLeft(void) {
	...
	_scroll_count++;
	if (_scroll_count >= 40)
		_scroll_count -= 40; //  -39< scroll_count<39
}
...
void LCD::setCursor(uint8_t col, uint8_t row) // this can be called by the user but is also called before writing some characters.
{
	if (row > _numlines) {
	...
	if (offset > 39)
		offset -= 40; // if the display is autoscrolled this method does not work, however.
	if (offset < 0)
		offset += 40;
	offset |= high_bit;
	if (_chip != (row & 0b10)) {
...
void LCD::autoscroll(void) { //to count the number of times we scrolled; here we'd need to keep track of microseconds and divide. I'm not going there.
...
void LCD::commandBoth(uint8_t value) { //for many of the commands that need to be sent twice if 2 controller chips
	if (_en2 == 255) {
...
void LCD::write(uint8_t value) { //print calls  this to send characters to the LCD
	if ((_scroll_count != 0) || (_setCursFlag != 0))
		setCursor(_x, _y); //first we call setCursor and send the character
	if ((value != '\r') && (value != '\n'))
		send(value, HIGH);
	...
	if (_y >= _numlines)
		_y = 0; //wrap last line up to line 0
}
...
void LCD::write4bits(uint8_t value) { //still used during init
	...
	byte en = _enable_pin;
	if ((_en2 != 255) && (_chip))
		en = _en2; // 4x40 LCD with 2 controller chips with separate enable lines if we called w 2 enable pins and are on lines 2 or 3 enable chip 2
	digitalWrite(en, HIGH); // enable pulse must be >450ns
	digitalWrite(en, LOW);
}
