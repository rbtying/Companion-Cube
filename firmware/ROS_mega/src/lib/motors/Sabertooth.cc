...
void Sabertooth::setSpeed(motor_data * data) {
	int8_t left = constrain(data->leftSpeed, -127, 127);
	int8_t right = constrain(data->rightSpeed, -127, 127);
	setSpeed(left, right, data->leftFwd, data->rightFwd);
}
...
void Sabertooth::setM2Speed(int8_t spd) {
	m_left_speed = spd;
	if (spd >= 0)
...
void Sabertooth::setM1Speed(int8_t spd) {
	m_right_speed = spd;
	if (spd >= 0)
