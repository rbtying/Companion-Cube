...
void OpcodeController::sendMessage(uint8_t op, uint8_t * contents, uint8_t length) {
...
uint8_t OpcodeController::checksum(uint8_t op, uint8_t * contents, uint8_t length) {
	uint32_t temp = op;
	for (uint8_t i = 0; i < length; i++) {
		temp += contents[i];
	}
...
void OpcodeController::sendEncoderMessage(uint8_t op, encoder_data * d, uint8_t out) {
	uint8_t buf[7];
